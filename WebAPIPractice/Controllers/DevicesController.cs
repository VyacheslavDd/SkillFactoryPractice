...
namespace WebAPIPractice.Controllers
{
    ...
    public class DevicesController : ControllerBase
    {
        ...
        [HttpDelete]
        [Route("delete/{id}")]
        public async Task<ActionResult> DeleteDevice([FromRoute] Guid id)
        {
            var device = await _devices.GetDeviceById(id);
            if (device == null)
            {
                return StatusCode(400, $"Устройства с идентификатором {id} не существует!");
            }
            await _devices.DeleteDevice(device);
            return StatusCode(200, $"Устройство с идентификатором {id} успешно удалено!");
        }
